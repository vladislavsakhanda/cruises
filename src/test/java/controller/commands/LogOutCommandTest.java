...
public class LogOutCommandTest {
    ...
    public void doGetTest() throws ServletException, IOException {
        String language = "en";
        when(session.getAttribute("lang")).thenReturn(language);

        command.doGet();

        // Verify that the session was invalidated and the language attribute was set again
        verify(session).invalidate();
        verify(request.getSession()).setAttribute("lang", language);
    }
